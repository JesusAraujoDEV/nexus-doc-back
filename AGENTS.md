# AGENTS.md — nexus-doc-back IA Entry
...
REST API backend for NexusDoc: Express 5 service over PostgreSQL via Sequelize, with JWT/Passport authentication, Joi request validation, and an OpenAPI surface published through swagger-ui.

Full specification: [`docs/spec.md`](docs/spec.md) — not yet written.

## Stack

| Layer | Tech |
|-------|------|
| Runtime | Node.js (CommonJS), `server.js` entry |
| HTTP | Express 5, `cors`, `express-rate-limit` |
| Auth | Passport (local + JWT), `jsonwebtoken`, `bcrypt` |
| Validation | Joi schemas (`schemas/`) |
| Errors | `@hapi/boom` |
| Data | PostgreSQL (`pg`) + Sequelize 6, `sequelize-cli` migrations |
| API docs | `swagger-jsdoc` + `swagger-ui-express` |
| Tooling | ESLint, nodemon, Docker |

Locked decisions (no re-discuss): no TypeScript, no ORM other than Sequelize, no GraphQL, migrations only via `sequelize-cli` (never `sync({ alter })`).

## Architecture rules

- **Request flow**: `routes/` → `middlewares/` (auth, validation) → `controllers/` → `services/` → `db/models/`.
- **No business logic in controllers.** Controllers parse the request, call a service, shape the response; all domain logic lives in `services/`.
- **No direct model access outside services.** Controllers never import from `db/models/`.
- **Validation at the boundary**: every mutating route validates its body/params with a Joi schema from `schemas/` before reaching the controller.
- **Errors via Boom**: services throw Boom errors; the error middleware is the only place that formats an HTTP error response. Never swallow.
- **Schema changes only through migrations** in `db/migrations/`; a model edit without its migration is incomplete.

## Code quality rules (MANDATORY)

Canonical source: [`standards/code-quality.md`](standards/code-quality.md). Summary:

- One symbol per file (one controller / service / model / schema).
- File size limits — controller 150, service 200, model 150, middleware 80.
- Function 30 lines max, complexity 10 max, nesting 3 max.
- Guard clauses + early returns. No `else` after `return`.
- Errors surfaced as Boom errors; never swallowed.

Enforced via ESLint (`npm run lint`). Refuse to write violating code; refactor instead. The generic core in `standards/code-quality.md` is written for a TypeScript/Biome stack; where it conflicts with this section, this section wins — record the divergence in `docs/DEVIATIONS.md`.
...
libs/                # shared infrastructure (sequelize instance, etc.)
utils/auth/          # Passport strategies, JWT helpers
swagger/             # OpenAPI definition
standards/           # code-quality core + project rules
docs/                # crew documentation tree
```
...
npm run lint                 # eslint
npm run migrations:generate  # sequelize-cli migration:generate --name <name>
npm run migrations:run       # apply migrations
npm run migrations:revert    # undo last migration
```

No test runner is configured (`npm test` is a stub) — `QA` owns closing that gap.
...
| `docs/work/` | Historical change log (immutable) |
| `standards/` | Code-quality core + project rules |
